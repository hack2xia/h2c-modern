...
**headers to curl** — paste a raw HTTP request message, get a curl command line that reproduces it
on the wire as faithfully as a command line can — and refuses or warns explicitly where it can't.
...
Requires **Deno ≥ 2.9.5** (the version pinned in CI and the Docker image).
...
Every input falls into exactly one of three classes:

| Class       | Meaning                                                      | CLI behavior                                  |
| ----------- | ------------------------------------------------------------ | --------------------------------------------- |
| **exact**   | command reproduces the original wire bytes                   | command on stdout, exit 0                     |
| **warning** | command generated, with caveats printed alongside            | command on stdout, warnings on stderr, exit 0 |
| **refused** | cannot be expressed faithfully / clearly broken — no command | message on stderr, exit 1                     |

**refused** (clear errors / impossible to express faithfully): empty request; request line with
leading/trailing whitespace or consecutive/missing spaces (curl rebuilds request lines with single
spaces — silent trimming would change the wire bytes); request line that isn't
...
**not** warned about); absolute-form request line (the URL is used directly; an extra warning is
added if it disagrees with the `Host` header); obs-folded headers (unfolded per RFC); a header name
that isn't a valid RFC 7230 field-name token (passed through verbatim — servers may reject or ignore
such a header line); a header name with surrounding whitespace (stripped with a warning); a header
value made only of whitespace (curl cannot reproduce a pure-OWS field value, sent as an empty
header); asterisk-form `OPTIONS *` (target sent verbatim via `--request-target`, requires curl ≥
7.55); duplicate `Content-Length` with identical values (each occurrence passed through — curl sends
them one by one); `Content-Length` disagreeing with the actual body byte count (the declared value
is passed through verbatim, reproducing the original bytes including the mismatch; declared > actual
...
All original headers — including `Host`, `Content-Length`, `User-Agent`, `Accept-Encoding`,
`Authorization` — are passed through **as `-H` in the original order**. No dedicated options (`-A`,
`--user`, `--compressed`) are used. Wire experiments with curl 8.x showed that a `-H` header
replaces curl's internally generated header of the same name by argv position (Host / User-Agent /
Accept / Content-Length / Content-Type), without duplicates — so `-H` is the only way to preserve
the header values, their whitespace forms, **and** their original order at the same time. Dedicated
options would reorder the header block and cannot control value whitespace.

- `Content-Length` with a valid numeric value is passed through in place (including values that
  disagree with the body — the wire bytes then match the original message exactly, mismatch
  included; see the warning list). A non-numeric CL falls back to curl's recomputation.
- A bodyless request that declared `Content-Length: 0` gets that header sent via
  `-H 'Content-Length: 0'` (wire-verified to be emitted even without a body); a bodyless `POST`
  additionally gets `--request POST` to keep the method (without it curl sends GET).
- `Cookie` always goes through `-H` as well — curl interprets a `--cookie` argument without `=` as a
  **filename** and tries to read a local cookie file (an unnecessary local file read and credential
  risk).
- **Default header suppression**: if the request lacks `Accept` / `User-Agent` and default headers
  ...
  `Content-Type: application/x-www-form-urlencoded`: if the original request has a body but no
  `Content-Type`, `-H 'Content-Type:'` is appended to clear it.
- **Method**: `HEAD` (no body) → `--head`; `GET` (no body) → default; others → `--request` (original
  case preserved); `GET`/`HEAD` with a body and bodyless `POST` → `--request` as above.
- **Body**: normal → `--data-raw` (byte-for-byte equivalent to `--data-binary`, but **without curl's
  ...
  and uploaded); `multipart/form-data` → also sent as a whole via `--data-raw` with the
  `Content-Type` header passed through verbatim (see below).
...
## Behavior changes in 0.4.1

This round tightened fidelity, guided by wire captures with curl 8.x:

- **No more dedicated header options**: the output no longer contains `-A` (`--user-agent`),
  `--user`, or `--compressed`. Every original header is passed through via `-H` in its original
  position — including `Host` and `Content-Length`, which used to be skipped (Host built the URL, CL
  was left for curl to recompute). The URL still comes from the Host header.
- **Content-Length is preserved, not recomputed**: a numeric CL is sent via `-H` as declared (even
  when it disagrees with the body — the wire bytes then match the original message exactly, and a
  warning points out the mismatch). A bodyless `Content-Length: 0` is sent via `-H`; a bodyless
  `POST` uses `--request POST` instead of the old `--data-raw ''` trick.
- **Stricter request-line validation**: leading/trailing whitespace and consecutive/missing spaces
  are refused (previously silently collapsed). Mixed line endings (CRLF + bare LF) produce a
  warning; pure LF input does not.
- `Authorization: Basic` goes through `-H` instead of `--user`.
